...
public class RequestParamCheckAop {
    ...
    public Object doAround(ProceedingJoinPoint pjp) throws Throwable {
        ...
        for (int i = 0; i < parameterAnnotations.length; i++) {
            for (int j = 0; j < parameterAnnotations[i].length; j++) {
                ...
                if (parameterAnnotations[i][j] != null && parameterAnnotations[i][j] instanceof RequestParamValidate && ((RequestParamValidate) parameterAnnotations[i][j]).notNull()) {
    ...
    private void paramIsNull(String paramName, Object value, String parameterType) {
        if (value == null || "".equals(value.toString().trim())) {
            throw new ServletRequestParamIsNullException(paramName, parameterType);
        }
    }
